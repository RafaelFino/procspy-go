#!/bin/bash
# install-server.sh
# Script de instalação do Procspy Server para Linux usando systemd
# Requer privilégios de root
#
# Uso:
#   sudo ./install-server.sh [opções]
#
# Opções:
#   -b, --binary PATH       Caminho para o binário procspy-server
#   -c, --config PATH       Caminho para o arquivo config-server.json
...
DEFAULT_BINARY_PATH="./bin/procspy-server"
DEFAULT_CONFIG_PATH="./etc/config-server.json"
...
show_help() {
    cat << EOF
${GREEN}=== Instalação do Procspy Server ===${NC}
...
Exemplos:
  ...
  sudo $0 --binary /tmp/procspy-server

  # Instalação com binário e config customizados
  sudo $0 -b /opt/builds/procspy-server -c /opt/configs/config-server.json

Pós-instalação:
  1. Configure firewall: sudo ufw allow 8080/tcp
  2. Configure proxy reverso (nginx/caddy)
  3. Configure HTTPS com certbot
...
echo -e "${GREEN}=== Instalação do Procspy Server ===${NC}"
echo ""
...
if [ ! -f "$BINARY_PATH" ]; then
    ...
    echo "  2. Especifique o caminho correto: $0 --binary /caminho/para/procspy-server"
    exit 1
fi
...
cp "$BINARY_PATH" "$INSTALL_DIR/procspy-server"
chmod +x "$INSTALL_DIR/procspy-server"
echo -e "${GREEN}✓ Binário instalado em $INSTALL_DIR/procspy-server${NC}"
...
if [ "$CONFIG_EXISTS" = true ]; then
    ...
    cp "$CONFIG_PATH" "$CONFIG_DIR/config-server.json"
    echo -e "${GREEN}✓ Configuração copiada para $CONFIG_DIR/config-server.json${NC}"
else
    echo -e "${CYAN}Criando configuração padrão...${NC}"
    cat > "$CONFIG_DIR/config-server.json" <<EOF
{
    "debug": false,
    "log_path": "$LOG_DIR",
    "db_path": "$DATA_DIR",
...
EOF
    echo -e "${GREEN}✓ Configuração padrão criada em $CONFIG_DIR/config-server.json${NC}"
    echo -e "${YELLOW}  IMPORTANTE: Edite a configuração antes de usar${NC}"
    echo -e "${YELLOW}  Execute: sudo micro $CONFIG_DIR/config-server.json${NC}"
fi
...
[Unit]
Description=Procspy Server - API e armazenamento de telemetria
...
ExecStart=$INSTALL_DIR/procspy-server $CONFIG_DIR/config-server.json
Restart=always
RestartSec=5
...
echo "  Binário:      $INSTALL_DIR/procspy-server"
echo "  Configuração: $CONFIG_DIR/config-server.json"
echo "  Banco de dados: $DATA_DIR"
echo "  Logs:         $LOG_DIR"
echo "  Service file: $SERVICE_FILE"
echo ""
echo -e "${YELLOW}Próximos Passos:${NC}"
echo ""
echo "  ${CYAN}1. Configure o firewall:${NC}"
echo "     sudo ufw allow 8080/tcp"
echo ""
echo "  ${CYAN}2. Configure proxy reverso (Nginx):${NC}"
echo "     sudo cp etc/nginx-procspy.conf /etc/nginx/sites-available/procspy"
echo "     sudo ln -s /etc/nginx/sites-available/procspy /etc/nginx/sites-enabled/"
echo "     sudo nginx -t && sudo systemctl reload nginx"
echo ""
echo "  ${CYAN}3. Configure proxy reverso (Caddy - alternativa):${NC}"
echo "     sudo cp etc/Caddyfile /etc/caddy/Caddyfile"
echo "     sudo systemctl reload caddy"
echo ""
echo "  ${CYAN}4. Configure HTTPS (com Nginx):${NC}"
echo "     sudo certbot --nginx -d seu-dominio.com"
echo ""
echo "  ${CYAN}5. Edite a configuração:${NC}"
echo "     sudo micro $CONFIG_DIR/config-server.json"
echo "     sudo systemctl restart $SERVICE_NAME"
echo ""
echo -e "${GREEN}✓ Procspy Server instalado com sucesso!${NC}"
