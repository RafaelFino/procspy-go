...
func (c *Command) ToLog() string {
	...
	if err != nil {
		log.Printf("[domain.Command.ToLog] Failed to marshal command to JSON: %v", err)
		return ""
	}
	return string(ret)
}
func (c *Command) ToJson() string {
	...
	if err != nil {
		log.Printf("[domain.Command.ToJson] Failed to marshal command to JSON: %v", err)
		return ""
	}
	return string(ret)
}
...
func CommandFromJson(jsonString string) (*Command, error) {
	...
	if err != nil {
		log.Printf("[domain.CommandFromJson] Failed to unmarshal command from JSON: %v", err)
		return nil, err
	}
	return ret, nil
}
